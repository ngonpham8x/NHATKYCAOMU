...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (firebaseUser: User | null) => {
      setTimeout(async () => {
        ...
        // 1. Fast Cache Hydration (0ms Instant Entry)
        try {
          const cachedStr = localStorage.getItem(`${USER_PROFILE_CACHE_PREFIX}${firebaseUser.uid}`);
          if (cachedStr) {
            const cachedProfile: UserProfile = JSON.parse(cachedStr);
            setCurrentUser(cachedProfile);
            setActiveViewingUserId(cachedProfile.parentUserId || cachedProfile.uid);
            if (cachedProfile.settings) {
        ...
        // 2. Subscribe to real-time permission changes
        unsubPermission = subscribeToEmailAccessPermission(email, async (permissionInfo) => {
          try {
            ...
            try {
              localStorage.setItem(`${USER_PROFILE_CACHE_PREFIX}${firebaseUser.uid}`, JSON.stringify(profile));
            } catch (e) {}
  ...
  // Direct login for preview/fallback when popups are blocked in iframe
  const handleDirectLogin = (email: string, role: 'admin' | 'user' = 'admin') => {
    const profile: UserProfile = {
      uid: `user_${email.replace(/[@.]/g, '_')}`,
      email: email.toLowerCase(),
      displayName: email.includes('bhttq3') ? 'Phạm Duy Ngôn (Admin)' : 'Chủ Vườn Cao Su',
      photoURL: '',
      role,
    ...
    showToast(`Đã đăng nhập thành công với vai trò ${role === 'admin' ? 'Admin' : 'Chủ Vườn'}!`, 'success');
  };
    ...
    <div className={`min-h-screen flex flex-col bg-slate-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors font-sans pb-16 lg:pb-8 ${viewMode === 'desktop' ? 'w-full overflow-x-auto' : ''}`}>
      ...
      <div className={`flex flex-1 w-full relative ${viewMode === 'desktop' ? 'overflow-x-auto' : ''}`}>
        ...
        <main className={`flex-1 w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5 min-w-0 transition-all duration-300 ${viewMode === 'desktop' ? 'min-w-[960px] md:min-w-[1150px] overflow-x-auto' : ''}`}>
          ...
          {activeTab === 'analytics' && <AnalyticsTab records={records} settings={settings} />}
